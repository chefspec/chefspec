module ChefSpec
  module ExampleHelpers
    ...
    def recipe_renders_template(options)
      options = {:missing => false, :another_cookbook => false, :partial => false}.merge(options)
      ...
      write_file "cookbooks/#{template_cookbook}/templates/default/#{template_name}", %Q{
        ...
        platform: <%= @platform %>#{', <%= render "partial.conf.erb" %>' if options[:partial]}
      }.gsub(/^\s+/, '')

      if options[:partial]
        write_file "cookbooks/#{template_cookbook}/templates/default/partial.conf.erb", %q{foo: <%=@platform %>}.gsub(/^\s+/, '')
      end
    end
